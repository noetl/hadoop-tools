...
cd /usr/lib
sudo aws s3 cp s3://noetl-provisioning-us-west-2/emr-4.7.1/zeppelin.tar.gz /usr/lib/ .
echo "Installing Zeppelin...."
sudo tar xzf zeppelin.tar.gz
sudo rm -rf zeppelin.tar.gz

echo "Configuring dirs for Zeppelin...."
sudo mkdir -p /mnt/var/log/zeppelin /mnt/var/lib/zeppelin/notebook /mnt/var/lib/zeppelin/webapps /mnt/var/run/zeppelin
sudo chown -R hadoop:hadoop /mnt/var/log/zeppelin /mnt/var/lib/zeppelin /mnt/var/run/zeppelin /usr/lib/zeppelin/conf
sudo su - hadoop -c 'cp -R /usr/lib/zeppelin/notebook/* /mnt/var/lib/zeppelin/notebook/'
